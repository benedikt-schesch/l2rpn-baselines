...
class OptimCVXPY(BaseAgent):
    ...
    def __init__(
        ...
        self._penalty_curtailment_safe: cp.Parameter = cp.Parameter(
            value=penalty_curtailment_safe, nonneg=True
        )
        self._penalty_redispatching_safe: cp.Parameter = cp.Parameter(
            value=penalty_redispatching_safe, nonneg=True
        )
        self._penalty_storage_safe: cp.Parameter = cp.Parameter(
            value=penalty_storage_safe, nonneg=True
        )

        self._weight_redisp_target: cp.Parameter = cp.Parameter(
            value=weight_redisp_target, nonneg=True
        )
        self._weight_storage_target: cp.Parameter = cp.Parameter(
            value=weight_storage_target, nonneg=True
        )
        self._weight_curtail_target = cp.Parameter(
            value=weight_curtail_target, nonneg=True
        )
        ...
        self._alpha_por_error: cp.Parameter = cp.Parameter(
            value=alpha_por_error,
            nonneg=True,
        )
        self.nb_max_bus: int = 2 * env.n_sub
        self._storage_power_obs: cp.Parameter = cp.Parameter(value=0.0)
        ...
        for bus_id in range(self.nb_max_bus):
            SoC[bus_id] = (
                0.5 * self._storage_setpoint[env.storage_to_subid == bus_id].sum()
            )
        ...
        if powerlines_x.shape[0] != env.n_line:
            raise ValueError(
                "The number of lines in the grid is not the same as the number "
                "of lines in provided lines_x"
            )
        if np.any(powerlines_x <= 0.0):
    ...
    def reset(self, obs: BaseObservation):
        ...
        if conv_:
            ...
            self.logger.warning(
                "Impossible to intialize the OptimCVXPY "
    ...
    def _update_th_lim_param(self, obs: BaseObservation):
        ...
        index_disc = obs.v_or == 0.0
        self._th_lim_mw.value[index_disc] = (
            0.001 * (obs.thermal_limit * self._v_ref)[index_disc] * np.sqrt(3.0)
        )
    ...
    def compute_optimum_unsafe(self):
        ...
        inj_bus = (self.load_per_bus + storage) - (self.gen_per_bus - curtailment_mw)
        energy_added = (
        ...
        if has_converged:
            self.flow_computed[:] = f_or.value
            res = (curtailment_mw.value, storage.value, None)
    ...
    def _solve_problem(self, prob, solver_type=None):
        ...
        try:
            ...
            if np.isfinite(tmp_):
                ...
                self.logger.warning(
                    f"Problem with the optimization for {solver_type}, infinite value returned"
                )
                raise cp.error.SolverError("Infinite value")

        except cp.error.SolverError as exc_:
            self.logger.warning(
                f"Problem with the optimization for {solver_type}: {exc_}"
            )
            return False
    ...
    def to_grid2op(
        ...
        # storage
        if act.n_storage and np.any(np.abs(storage) > 0.0):
    ...
    def compute_optimum_safe(self, obs: BaseObservation, l_id=None):
        ...
        dispatch_after_this = self._past_dispatch
    ...
    def act(
        ...
        self._prev_por_error.value[prev_ok] = np.minimum(
            self.flow_computed[prev_ok] - obs.p_or[prev_ok], 0.0
        )
        ...
        if obs.rho.max() > self.rho_danger:
            ...
            # get back the grid2op representation
            act = self.to_grid2op(
                obs, curtailment, storage, redispatching, act, safe=True
            )
        else:
...
def make_agent(
    ...
    rho_danger: float = 0.95,
    rho_safe: float = 0.85,
    penalty_curtailment_unsafe: float = 0.0,
    penalty_redispatching_unsafe: float = 0.00,
    penalty_storage_unsafe: float = 0.0,
    penalty_curtailment_safe: float = 0.0,
    penalty_redispatching_safe: float = 0.0,
    weight_redisp_target: float = 1.0,
    weight_storage_target: float = 1.0,
...
def play_episode(env: Environment, agent: BaseAgent, n_episode: int = 10):
    for idx in range(n_episode):
        ...
        total_len = 0
        for i in tqdm(range(env.chronics_handler.max_episode_duration())):
            ...
            if done:
                break
        print(
            f"Episode {idx} done with reward = {total_reward} episode length = {total_len}/{env.chronics_handler.max_episode_duration()}"
        )
...
if __name__ == "__main__":
    ...
    runner_params = env.get_params_for_runner()
    runner_params["verbose"] = True
    runner = Runner(**runner_params, agentClass=None, agentInstance=make_agent(env))
    res = runner.run(
        nb_episode=6,
        pbar=True,
    )
    print(res, "\n")
    # env = get_env()
    # play_episode(env, make_agent(env), n_episode=10)
    # env = get_env()
    # play_episode(env, DoNothingAgent(env.action_space), n_episode=10)
